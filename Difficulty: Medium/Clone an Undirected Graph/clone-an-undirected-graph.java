...
*/
class Solution {
    private Node dfs(Node node, Set<Node> vis, Map<Node,Node> mp) {
        if(node==null) return null;
        vis.add(node);
        Node copyNode=new Node(node.val);
        mp.put(node,copyNode);
        for(Node neighbour:node.neighbors) {
            if(!vis.contains(neighbour)) dfs(neighbour,vis,mp);
            copyNode.neighbors.add(mp.get(neighbour)); // even if the node is already visited still it's an immediate neighbour node so should be added in the neighbors list 
